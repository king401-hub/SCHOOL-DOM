...
class SchoolFee(models.Model):
    ...
    # Bill/Invoice tracking - set only when this fee was generated by the
    # Generate Bill system (finance.Bill), mutually exclusive with class_fee.
    ...
    invoice_number = models.CharField(max_length=40, blank=True, null=True, unique=True, db_index=True)
...
class PaymentReceiptLink(models.Model):
    ...
    BILL = "bill"
    INVOICE = "invoice"
    TYPE_CHOICES = [(RECEIPT, "Receipt"), (BILL, "Bill"), (INVOICE, "Invoice")]
...
class Bill(models.Model):
    ...
    # Sent/Viewed/Partially Paid/Paid/Overdue are NOT stored here - they are
    # computed live from the linked SchoolFee rows (see finance.services
    # bulk_bill_status_counts), following this codebase's existing
    # "compute, don't duplicate" convention.

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    tenant = models.ForeignKey("core.SchoolTenant", on_delete=models.CASCADE, related_name="bills")
    title = models.CharField(max_length=150)
    classes = models.ManyToManyField("academic.Class", related_name="bills", blank=True)
    academic_year = models.ForeignKey(
        "academic.AcademicYear", on_delete=models.SET_NULL, null=True, blank=True, related_name="bills"
    )
    term = models.ForeignKey(
        "academic.Term", on_delete=models.SET_NULL, null=True, blank=True, related_name="bills"
    )
    ...
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, null=True, blank=True, on_delete=models.SET_NULL, related_name="created_bills"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def __str__(self):
        return f"Bill({self.title} - {self.status})"
...
class BillItem(models.Model):
    """One fee line-item template within a Bill (description + amount),
    applied identically to every student targeted at publish time. Not a
    per-student row - that's SchoolFee."""
