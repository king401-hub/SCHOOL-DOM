...
import TimetableSettingsDialog from "./components/timetable/TimetableSettingsDialog";
...
function AdminFinanceScreen({
  ...
  const renderStudentPaymentRow = (row) => (<tr key={row.id}><td>{row.name}<small>{row.student_id}</small>{Number(row.outstanding_bill_amount || 0) > 0 ? <small>Includes {formatFinanceAmount(row.outstanding_bill_amount)} outstanding bill{row.outstanding_bill_from ? ` from ${row.outstanding_bill_from}` : ""}</small> : null}</td><td>{row.class_name}</td><td><span className={`finance-status status-${row.payment_status}`}>{row.payment_status}</span></td><td>{formatFinanceAmount(row.expected_amount)}</td><td>{formatFinanceAmount(row.amount_paid)}</td><td>{formatFinanceAmount(row.remaining_balance)}</td></tr>);
  const renderStudentFeeRow = (fee) => (<tr key={fee.id}><td>{fee.student_name}<small>{fee.student_identifier}</small></td><td>{fee.class_label}</td><td>{fee.title}{fee.is_outstanding_bill ? <small>Outstanding bill{fee.outstanding_from ? ` from ${fee.outstanding_from}` : ""}</small> : null}</td><td>{formatFinanceAmount(fee.amount)}</td><td>{formatFinanceAmount(fee.amount_paid)}</td><td>{formatFinanceAmount(fee.remaining_balance)}</td><td><span className={`finance-status status-${fee.payment_status || fee.status}`}>{fee.payment_status || fee.status}</span></td><td><button type="button" className="table-action" onClick={() => startEditStudentFee(fee)}>Edit</button></td></tr>);
...
function AdminTimetablesScreen({ data = {}, loading, error, onRetry, onCreate, onUpdate, onDelete, onDeleteClass, onSaveSettings, onGenerate }) {
  ...
  const [dialog, setDialog] = useState(null); // the add / edit popup
  const [dialogOpen, setDialogOpen] = useState(false);
  const [classView, setClassView] = useState(null); // the "view a class timetable" popup
  const [classViewOpen, setClassViewOpen] = useState(false);
  ...
  const openDialog = (next) => {
    counter.current += 1;
    setDialog({ ...next, nonce: counter.current });
    setDialogOpen(true);
  };
  ...
  const handleSaved = (text, classId) => {
    setDialogOpen(false);
    counter.current += 1;
    setNotice({ id: counter.current, tone: "success", text });
    // follow a lesson added to another class on the board, but not while a class popup is open
    if (!classViewOpen) setJumpTo({ classId, nonce: counter.current });
  };

  const openClassView = (classId) => {
    counter.current += 1;
    setClassView({ id: classId, nonce: counter.current });
    setClassViewOpen(true);
  };

  // Delete a whole class timetable from the list under the grid (asks first).
  const handleDeleteClass = async ({ label, entryIds, count }) => {
    const ok = await confirm({
      title: `Delete the ${label} timetable?`,
      message: `All ${count} ${count === 1 ? "lesson" : "lessons"} for ${label} will be removed. You can rebuild it with Generate, but any lessons you added or changed by hand will be lost.`,
    ...
    try {
      await onDeleteClass?.(entryIds);
      setNotice({ id, tone: "success", text: `Deleted the ${label} timetable (${count} ${count === 1 ? "lesson" : "lessons"}).` });
    } catch (actionError) {
      setNotice({ id, tone: "error", text: actionError?.message || `Could not delete the ${label} timetable.` });
    }
  };

  const handleSettingsSaved = (text) => {
    ...
    <section className="screen-grid">
      ...
      <article className="app-panel tt-panel">
        <TimetableBoard
          ...
          notice={notice}
          jumpTo={jumpTo}
          onOpenEntry={(entry) => openDialog({ mode: "form", entry })}
      ...
      <article className="app-panel tt-settings">
      ...
      <ClassTimetableDialog
        open={classViewOpen}
        view={classView}
        // while an edit popup is on top of it, Escape / backdrop clicks belong to that popup
        onClose={() => {
          if (!dialogOpen) setClassViewOpen(false);
        }}
        classes={classes}
        entries={entries}
        days={days}
        timeSlots={timeSlots}
        onEditEntry={(entry) => openDialog({ mode: "form", entry })}
        onAddAt={(slot) => openDialog({ mode: "form", prefill: slot })}
      />
      ...
      <div className="document-theme-layout">
        ...
        <div className="app-panel document-theme-preview">
          ...
          <p className="document-theme-preview-note">
            ...
            up this same theme - preview them from their own screens after saving. The Service Agreement is
            SchoolDom's own legal document and is not affected by this theme.
          </p>
        </div>
      </div>
    </section>
              ...
              {showFilters ? (
                <div className="directory-filter-fields">
                  <label className="panel-field">
                    ...
                    >
                      <option value="">All {groupLabels.plural.toLowerCase()}</option>
                      {classes.map((item) => (
                        <option key={item.id} value={item.id}>{item.label}</option>
                      ))}
                    </select>
